  ...
  useEffect(() => {
    ...
    const getStandings = async () => {
      try {
        ...
        const standings = await useFetchData(`https://site.api.espn.com/apis/v2/sports/soccer/${filters.league}/standings`);
        ...
        <div className='standings' key={group.id}>
          ...
          {group.standings.map((team, index) => (
            <div className="team row" key={team.id}>
              <span className='rank'>{team.rank ? team.rank.number : index + 1}</span>
              ...
              <span>{team.stats[5].value}:{team.stats[4].value}</span>
              <span>{team.stats[2].displayValue}</span>
              <span><b>{team.stats[3].displayValue}</b></span>
            </div>
          ))}
